...
const getData = async () => {
  try {
    ...
    laptopDataArr.push(...data.laptop);
    renderLaptopData(laptopDataArr);
  } catch (error) {
...
let products = document.querySelector(".laptopProducts");
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = laptopDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderLaptopData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = laptopDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderLaptopData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = laptopDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderLaptopData(filtered);
});

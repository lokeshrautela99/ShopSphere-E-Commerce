...
const getData = async () => {
  try {
    ...
    mobileDataArr.push(...data.mobile);
    renderMobileData(mobileDataArr);
  } catch (error) {
...
let products = document.querySelector(".mobileProducts");
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = mobileDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderMobileData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = mobileDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderMobileData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = mobileDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderMobileData(filtered);
});

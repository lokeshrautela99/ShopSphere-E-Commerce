...
const getData = async () => {
  try {
    ...
    gamingDataArr.push(...data.gaming);
    renderGamingData(gamingDataArr);
  } catch (error) {
...
let products = document.querySelector(".gamingProducts");
let renderGamingData = (data) => {
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = gamingDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderGamingData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = gamingDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderGamingData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = gamingDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderGamingData(filtered);
});

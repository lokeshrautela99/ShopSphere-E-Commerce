...
// For user = https://e-commerce-data-base.onrender.com/users
// For Cart = https://e-commerce-data-base.onrender.com/cart
// For wishtlish = https://e-commerce-data-base.onrender.com/wishlist

let price = JSON.parse(localStorage.getItem("Price")) || 0;
let cartCount = JSON.parse(localStorage.getItem("cartCount")) || 0;

let homeDecorDataArr = [];
const getData = async () => {
  try {
    let res = await fetch(`https://e-commerce-data-base.onrender.com/productData`);
    let data = await res.json();
    homeDecorDataArr.push(...data.homeDecor);
    renderHomeDecorData(homeDecorDataArr);    
  } catch (error) {
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = homeDecorDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderHomeDecorData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = homeDecorDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderHomeDecorData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = homeDecorDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderHomeDecorData(filtered);
});

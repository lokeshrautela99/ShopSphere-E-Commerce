...
let products = document.querySelector(".kidProducts");
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = kidDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderKidData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = kidDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderKidData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = kidDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderKidData(filtered);
});

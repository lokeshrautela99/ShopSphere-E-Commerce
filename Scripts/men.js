...
let renderMenData = (data) => {
  let products = document.querySelector(".menProducts");
...
document.getElementById("sortLH").addEventListener("click", () => {
  let sortedLH = menDataArr.sort((a, b) => a.deal_price - b.deal_price);
  renderMenData(sortedLH);
});
document.getElementById("sortHL").addEventListener("click", () => {
  let sortedHL = menDataArr.sort((a, b) => b.deal_price - a.deal_price);
  renderMenData(sortedHL);
});
...
category1.addEventListener("click", () => {
  let filtered = menDataArr.filter((ele) => {
    return ele.category === category1.innerText;
  });
  renderMenData(filtered);
});
